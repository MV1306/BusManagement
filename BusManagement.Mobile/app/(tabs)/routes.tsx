...
export default function RoutesScreen() {
  ...
  const load = useCallback(async (p: number, s: string) => {
    ...
    try {
      ...
      setRoutes(prev => p === 1 ? r.items : [...prev, ...r.items]);
  ...
  const loadMore = () => {
    if (loading || routes.length >= totalCount) return;
    const next = page + 1;
    setPage(next);
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      <View style={styles.detailHeader}>
        ...
        <View style={styles.detailTitleWrap}>
          <View style={styles.routeCodeBadge}><Text style={styles.routeCodeBadgeText}>{selected.routeCode}</Text></View>
                ...
                <View key={rs.routeStopId} style={styles.timelineRow}>
                  <View style={styles.timelineLeft}>
                    <View style={[styles.dot,
                      rs.isFirstStop && styles.dotFirst,
                      rs.isLastStop && styles.dotLast,
                    ]} />
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListFooterComponent={loading ? <ActivityIndicator color="#1a56db" style={{ padding: 16 }} /> : null}
          renderItem={({ item: r }) => (
            <TouchableOpacity style={styles.routeCard} onPress={() => openDetail(r)}>
              <View style={styles.routeCardLeft}>
                <View style={styles.routeCodeBadge}><Text style={styles.routeCodeBadgeText}>{r.routeCode}</Text></View>
...
const styles = StyleSheet.create({
  ...
  countText: { fontSize: 12, color: '#6b7280', marginBottom: 8, fontWeight: '600' },
  routeCard: {
